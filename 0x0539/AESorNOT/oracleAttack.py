...
def call_oracle(up_cipher):
    p = remote('challenges.0x0539.net', 6767)
    c = base64.b64encode(bytes.fromhex(up_cipher)).decode()
    #log.info("sending: "+c)
    p.sendline(c)
    resp = p.recv(512).decode("latin-1")
    #log.info("receving")
    print(resp)
    p.close()
    if "Failed" in resp :
...
if __name__ == "__main__":
    ...
    cipher = base64.b64decode(args.message).hex()
    run(cipher, 16)
